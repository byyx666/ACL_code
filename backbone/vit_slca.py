...
@register_model
def vit_base_patch16_224_slca(pretrained=False, **kwargs):
...
@register_model
def vit_base_lora_patch16_224_slca(pretrained=False, lora_rank=4, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, lora_rank=lora_rank, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def vit_base_patch16_224_in21k_slca(pretrained=False, **kwargs):
...
@register_model
def vit_base_lora_patch16_224_in21k_slca(pretrained=False, lora_rank=4, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
