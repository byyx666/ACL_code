...
class Learner(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        if "warm" in self.args and self.args["warm"] is True:
            ...
            warm_optimizer = self.get_optimizer(lr=self.warm_lr)
            scheduler = None
    ...
    def _train_warm(self, train_loader, test_loader, optimizer, scheduler):
        ...
        with torch.no_grad():
            for i, batch in enumerate(train_loader):
                ...
                embedding = self._network(data, task_id=self._cur_task, train=True)["pre_logits"]
                embedding_list.append(embedding.cpu())
                label_list.append(label.cpu())
        embedding_list = torch.cat(embedding_list, dim=0)
        label_list = torch.cat(label_list, dim=0)

        self.proto_list = torch.zeros(self._total_classes - self._known_classes, embedding_list.size(1)).to(self._device)
        class_list = torch.unique(label_list)
        for class_index in class_list:
            data_index = (label_list == class_index).nonzero().squeeze(-1)
            embedding = embedding_list[data_index]
            proto = embedding.mean(0)
            self.proto_list[class_index - self._known_classes] = proto
        print(proto.size())
