...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self._known_classes > 0:
            ...
            self.test_loader_old = DataLoader(test_dataset_old, batch_size=self.batch_size, shuffle=False, num_workers=num_workers)
        else:
    ...
    def _compute_mean(self, model):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            if self.args["ca_storage_efficient_method"] == 'covariance':
                ...
                # have changed 1e-4 to 1e-3, to fix a bug
                self.cls_cov[class_idx] = torch.cov(features_per_cls.T) + (torch.eye(self.cls_mean[class_idx].shape[-1]) * 1e-3).to(self._device)
            elif self.args["ca_storage_efficient_method"] == 'variance':
                features_per_cls = vectors
                # print(features_per_cls.shape)
                self.cls_mean[class_idx] = features_per_cls.mean(dim=0).to(self._device)
                self.cls_cov[class_idx] = torch.diag(torch.cov(features_per_cls.T) + (torch.eye(self.cls_mean[class_idx].shape[-1]) * 1e-3).to(self._device))
            ...
            if self.args["ca_storage_efficient_method"] in ['covariance', 'variance']:
                ...
                for class_idx in range(self._total_classes):
                    for cluster in range(len(self.cls_mean[class_idx])):
                        ...
                        if var.mean() == 0:
                            continue
                        m = MultivariateNormal(mean.float(), (torch.diag(var) + 1e-3 * torch.eye(mean.shape[0]).to(mean.device)).float())
                        sampled_data_single = m.sample(sample_shape=(num_sampled_pcls,))
                        sampled_data.append(sampled_data_single)
                        sampled_label.extend([class_idx] * num_sampled_pcls)
            else:
    ...
    def _train_warm(self, train_loader, test_loader, optimizer, scheduler):
        ...
        with torch.no_grad():
            for i, batch in enumerate(train_loader):
                ...
                data = data.to(self._device)
                label = label.to(self._device)
                embedding = self._network(data, adapter_id=self._cur_task, train=True)['features']
                embedding_list.append(embedding.cpu())
                label_list.append(label.cpu())
        embedding_list = torch.cat(embedding_list, dim=0)
        label_list = torch.cat(label_list, dim=0)
        ...
        for epoch in range(self.warm_epochs):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                embeddings = self._network(inputs, adapter_id=self._cur_task, train=True)['features']
